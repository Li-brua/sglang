...
class DSparkWorkerV2(BaseSpecWorker):
    ...
    def __init__(
        ...
        if get_disagg().enable_pdmux and not getattr(
            self.model_runner.model, "supports_pdmux_dspark_prefill", False
        ):
            raise NotImplementedError(
                "PDMux + DSPARK requires a target model that preserves DSPARK "
                "auxiliary hidden states across split prefill. DeepSeek-V4 is "
                "currently supported."
            )
    ...
    def update_pdmux_decode_attn_backend(self, stream_idx: int) -> None:
        """Switch both runners to the backend for the active PDMux stream."""
    ...
    def forward_batch_split_prefill(
        ...
        """Run one PDMux target segment and finalize DSpark on the last one."""
    ...
    def _finalize_prefill(
        ...
        """Inject target captures into draft KV after a complete prefill."""
