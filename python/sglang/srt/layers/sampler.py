...
class Sampler(nn.Module):
    def __init__(self):
        ...
        # Under PDMux the TP group is lane-dependent: get_tp_group() returns the
        # duplicate prefill communicator while the prefill lane is active. A
        # group cached here would make the prefill lane's sync share the decode
        # lane's communicator with a decode collective in flight, so that mode
        # resolves the group per call instead. DP attention keeps its cached
        # group: attn_tp_group is not lane-dependent, so it is already correct.
        self._resolve_tp_sync_group_per_call = (
            is_pdmux_enabled() and not is_dp_attention_enabled()
        )
